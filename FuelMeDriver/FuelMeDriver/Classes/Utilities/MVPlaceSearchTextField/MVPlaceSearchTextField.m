...
- (void)autoCompleteTextField:(MLPAutoCompleteTextField *)textField
 possibleCompletionsForString:(NSString *)string
        ...
        if(aQuery.length>0){
            ...
            CLLocationCoordinate2D start=CLLocationCoordinate2DMake(30.516863, -97.938383);
            CLLocationCoordinate2D end=CLLocationCoordinate2DMake(30.09868, -97.561489);

            GMSCoordinateBounds * bounds=[[GMSCoordinateBounds alloc]initWithCoordinate:end coordinate:start];
            ...
            [_placesClient autocompleteQuery:aQuery
                                    ...
                                    callback:^(NSArray *results, NSError *error) {
                                        if (error != nil) {
                                            ...
                                            [ErrorReporter recordError:error withDomainName:GOOGLEAutoComplete];
                                            handler(nil);
                                            return;
                                        }
                                        if(results.count>0){
